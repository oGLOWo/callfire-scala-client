...
trait ProductionRunscopeClientConnection extends ClientConnection {
  ...
  val connection: ActorRef = {
    for {
      Http.HostConnectorInfo(connector, _) <- IO(Http) ? Http.HostConnectorSetup(system.settings.config.getString("com.oglowo.callfire.runscope-api-host"), port = 443, sslEncryption = true)
    } yield connector
  }.await
}